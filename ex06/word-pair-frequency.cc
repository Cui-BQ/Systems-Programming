...
/* Reads stdin.  Writes to stdout each pair of words seen in the input
...
int main(int argc, char *argv[])
{
  ...
  int total_words = 0;
  /* Whatever this produces is our definition of "a pair of word." */

  while ((std::cin >> next_word.first) && (std::cin >> next_word.second))
    {
  ...
  for (auto const & [key,value]: word_map)
    {
      std::cout << std::setw(10) << key.first << std::setw(10) 
                << key.second << std::setw(10)
