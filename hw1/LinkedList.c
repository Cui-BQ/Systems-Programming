...
bool LinkedList_Pop(LinkedList *list, LLPayload_t *payload_ptr) {
  ...
  if (list == NULL || payload_ptr == NULL || list->num_elements == 0){
    return false;
  }
...
bool LLIterator_Remove(LLIterator *iter,
  ...
  if (iter->list == NULL || payload_free_function == NULL || iter == NULL || iter->node == NULL || iter->list->num_elements == 0) {
    return false;
  }
  ...
  if (iter->list->num_elements == 1) {
    ...
    LinkedListNode* freeMe = iter->node;
    iter->node = iter->node->next;
    free(freeMe);
    iter->list->num_elements--;
    return true;
