...
// A helper function that helps to find and optionally remove a key within a chain.
static bool FindOldKey(LLIterator* iter, HTKey_t key);
...
bool HashTable_Insert(HashTable *table,
  ...
  if (FindOldKey(iter, newkeyvalue.key)) {
    HTKeyValue_t *temp;
    LLIterator_Get(iter, (LLPayload_t*) &temp);
    oldkeyvalue->key = temp->key;
    oldkeyvalue->value = temp->value;
    temp->value = newkeyvalue.value;
    free(iter);
    return true;
  } else {
    table->num_elements++;
    HTKeyValue_t *ht = (HTKeyValue_t *) malloc(sizeof(HTKeyValue_t));
    ht->key = newkeyvalue.key;
    ht->value = newkeyvalue.value;
    LinkedList_Push(chain, (LLPayload_t*)ht);
    free(iter);
    return false;
  }
}
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  || iter->bucket_idx >= iter->ht->num_buckets || iter->ht->num_elements == 0){
    return false;
  }
  return true;
}
...
static bool FindOldKey(LLIterator* iter, HTKey_t key){
  HTKeyValue_t *current;
  while (LLIterator_IsValid(iter)){
    LLIterator_Get(iter, (LLPayload_t*) &current);
    if (current->key == key){
      return true;
    }
