...
static int WriteHeader(FILE *f,
  ...
  long unsigned int total = doctableBytes + memidxBytes;
  uint8_t *bytes = new uint8_t[total];
  if (fread(bytes, 1, total, f) != total) return kFailedWrite;
  for (long unsigned int i = 0; i < total; i++){
    crc.FoldByteIntoCRC(bytes[i]);
  }
  delete[] bytes;
...
static int WriteHashTable(FILE *f,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    LinkedList* currentBucket = ht->buckets[i];
    int numElts = LinkedList_NumElements(currentBucket);
    if (WriteHTBucketRecord(f, recordPos, numElts, bucketPos) == kFailedWrite) return kFailedWrite;
    if (numElts > 0) {
      int wbytes = WriteHTBucket(f, bucketPos, currentBucket, fn);
      if (wbytes == kFailedWrite) return kFailedWrite;
      bucketPos += wbytes;
    }
...
static int WriteHTBucket(FILE *f,
  ...
  for (int i = 0; i < numElts; i++) {
    ...
    int wbytes = fn(f, elementPos, static_cast<HTKeyValue_t*>(kv));
    if (wbytes == 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    elementPos += wbytes;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE *f,
  ...
  int16_t filenameBytes = strlen(static_cast<char*>(kv->value));  // you may want to change this
  ...
  if (fwrite(static_cast<char*>(kv->value), filenameBytes, 1, f) != 1) {
    return kFailedWrite;
  }
  ...
  return sizeof(DoctableElementHeader) + filenameBytes;  // you may want to change this
}
...
static int WriteDocidToPositionListFn(FILE *f,
  ...
  if (fwrite(&header, sizeof(DocIDElementHeader), 1, f) != 1) return kFailedWrite;
  ...
  return sizeof(DocIDElementHeader) + numPositions * sizeof(DocIDElementPosition);  // you may want to change this
}
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  int16_t wordBytes = strlen(static_cast<char*>(wp->word));  // you may want to change this
  ...
  // place in the file.
  if (fwrite(static_cast<char*>(wp->word), wordBytes, 1, f) != 1) return kFailedWrite;



  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(WordPostingsHeader) + wordBytes + htBytes;  // you may want to change this
}
}  // namespace hw3
