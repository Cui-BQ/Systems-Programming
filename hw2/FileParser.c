...
static void AddWordPosition(HashTable *tab, char *word,
  ...
  if (HashTable_Find(tab, hashKey, &kv)) {
    ...
    WordPositions* value = (WordPositions*)malloc(sizeof(WordPositions));
    char * thisWord = (char *) malloc(strlen(word) + 1);
    strncpy(thisWord, word, strlen(word) + 1);
    value->word = thisWord;
    value->positions = list;
    kv.value = value;
    HTKeyValue_t old_value;
    HashTable_Insert(tab, kv, &old_value);
  }
}
